...
void handlePresses();
void checkKeyboard();
void assignIncrementButtons(int *adjVar, int low, int up, int inc);
void incDecSpecials();
void unSplash();
void clearHeld();
...
inline int32_t filterNextL(int32_t in);
...
// POTS.ino
void getPots();
void lockPot(byte Pot);
boolean unlockedPot(byte Pot);
void adjustValues();
void getMenu();
...
void getDirCount();
void loadWaveshape();
boolean checkExtension(char ext[]);
void savePatch();
void loadPatch();
void loadProceed();
void setFolder();
...
// UI.ino
void updateLED();
void showValue(byte h, byte v, int number);
void updateMenu();
void updateValues();
